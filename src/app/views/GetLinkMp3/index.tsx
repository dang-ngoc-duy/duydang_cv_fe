    ...
    <div className="container gutter-top">
      <Box>
        <Intro title="Get Link Mp3">
          ...
          />
          <label className="mt-1 text-warning">{message}</label>
          <br />
          <Button
      ...
      {currType !== "" && (
        <Box className={classes.SongList}>
          {currType === "album" &&
            zingDtaAlbum !== undefined &&
            zingDtaAlbum?.items?.map((song, idx) => (
              <div key={idx} className={classes.Song}>
                ...
                <a
                  href={song.source["128"]}
                  target="_blank"
                  rel="noopener noreferrer"
                  download
                >
                  <IconButton
                    disabled={!song.source["128"]}
          ...
          {(currType === "bai-hat" || currType === "video-clip") &&
            zingDtaAudio !== undefined && (
              <div className={classes.Song}>
                <img
                  className="image"
                  src={zingDtaAudio?.thumbnail}
                  alt="thumbnail"
                />
                <div className="info">
                ...
                <a
                  href={zingDtaAudio.source["128"]}
                  target="_blank"
                  rel="noopener noreferrer"
                  download
                >
                  <IconButton
                    disabled={!zingDtaAudio.source["128"]}
                    className="btn-download"
                    color="primary"
                    aria-label="Tải về máy"
                  >
